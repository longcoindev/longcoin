...
Value masternode(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "count") return (int)longSendMasterNodes.size();
    ...
    if (strCommand == "current")
    {
        ...
        int winner = longSendPool.GetCurrentMasterNode(mod);
        if(winner >= 0) {
            return longSendMasterNodes[winner].addr.ToString().c_str();
        }
    ...
    {
        Object obj;
        BOOST_FOREACH(CMasterNodeVote& mv, longSendMasterNodeVotes) {
