...
uint256 hashGenesisBlock("0x00000be72712a924e5ecb78ee55c41682f7215f9bb2731858ab17a007056166d"); //mainnet

static CBigNum bnProofOfWorkLimit(~uint256(0) >> 10); // LongCoin: starting difficulty is 1 / 2^10
...
// create LongSend pools
CLongSendPool longSendPool;
CLongSendSigner longSendSigner;
std::vector<CMasterNode> longSendMasterNodes;
std::vector<CMasterNodeVote> longSendMasterNodeVotes;
...
{
    // LongCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    if (nHeight == 0) {
        ...
        // no block reward to allow difficulty to scale up and prevent instamining (6 hours)
        nSubsidy = 0;
...
static const int64 nTargetTimespan = 24 * 60 * 60; // LongCoin: 1 day
static const int64 nTargetSpacing = 1 * 60; // LongCoin: 2.5 minutes
...
unsigned int static LongGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, longcoin - LongGravity v2, written by Evan Duffield - evan@longcoin.org */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static LongGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, longcoin - LongGravity v3, written by Evan Duffield - evan@longcoin.org */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
            ...
            if (pindexLast->nHeight+1 >= 60000) { DiffMode = 4; }
            else if (pindexLast->nHeight+1 >= 30000) { DiffMode = 3; }
            else if (pindexLast->nHeight+1 >= 10000) { DiffMode = 2; }
        }

        if (DiffMode == 1) { return GetNextWorkRequired_V1(pindexLast, pblock); }
        else if (DiffMode == 2) { return GetNextWorkRequired_V2(pindexLast, pblock); }
        else if (DiffMode == 3) { return LongGravityWave(pindexLast, pblock); }
        else if (DiffMode == 4) { return LongGravityWave3(pindexLast, pblock); }
        return LongGravityWave3(pindexLast, pblock);
}
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckVotes) const
{
    ...
    // LongCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // LongCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        hashGenesisBlock = uint256("0x00000be72712a924e5ecb78ee55c41682f7215f9bb2731858ab17a007056166d");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block        
        const char* pszTimestamp = "03/Aug/2014 Longcoin released";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 0.5 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("049206840c6e646b59cb199e053bd959fbc55aab549979a5530e222c3e2f8cdf822e46a6f6f1687c8ab943cc27203e7e486b9a3c30b0b1db988a4680b316b9c769") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1407099160;
        ...
        assert(block.hashMerkleRoot == uint256("0x2c0f6fe48316d0dcc9a92a753f0f67170b3ea689c00623617bb26a0a6e174c35"));
        block.print();
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion >= longSendPool.MIN_PEER_PROTO_VERSION) {
    ...
    else if (strCommand == "verack")
    {
    ...
    else if (strCommand == "dseg") { //LongSend Election Get
        if (pfrom->nVersion < longSendPool.MIN_PEER_PROTO_VERSION) {
            return false;
        }

        int count = longSendMasterNodes.size()-1;
        int i = 0;

        BOOST_FOREACH(CMasterNode mn, longSendMasterNodes) {
    ...
    else if (strCommand == "dsee") { //LongSend Election Entry   
        if (pfrom->nVersion < longSendPool.MIN_PEER_PROTO_VERSION) {
            return false;
        }
        bool fIsInitialDownload = IsInitialBlockDownload();
        ...
        if(!longSendSigner.VerifyMessage(pubkey, vchSig, strMessage, errorMessage)){
        ...
        BOOST_FOREACH(CMasterNode& mn, longSendMasterNodes) {
            ...
            if(mn.vin == vin) {
                if(!mn.UpdatedWithin(MASTERNODE_MIN_MICROSECONDS)){
                    ...
                    if(count == -1)
                        RelayLongSendElectionEntry(vin, addr, vchSig, sigTime, pubkey, pubkey2, count, current, lastUpdated);
                }
        ...
        CTxOut vout = CTxOut(999.99*COIN, longSendPool.collateralPubKey);
        ...
        if(tx.AcceptableInputs(state, true)){
            ...
            longSendMasterNodes.push_back(mn);
    ...
    else if (strCommand == "dseep") { //LongSend Election Entry Ping 
        if (pfrom->nVersion < longSendPool.MIN_PEER_PROTO_VERSION) {
            return false;
        }
        bool fIsInitialDownload = IsInitialBlockDownload();
        ...
        BOOST_FOREACH(CMasterNode& mn, longSendMasterNodes) {
            ...
            if(mn.vin == vin) {
                ...
                std::string errorMessage = "";
                if(!longSendSigner.VerifyMessage(mn.pubkey2, vchSig, strMessage, errorMessage)){
                ...
                if(stop) {
                    if(mn.IsEnabled()){
                        mn.Disable();
                        mn.Check();
                        RelayLongSendElectionEntryPing(vin, vchSig, sigTime, stop);
                    }
                    return true;
    ...
    int64 nFees = 0;
    {
        ...
        if(bMasterNodePayment) {
            if(!pblock->MasterNodePaymentsEnforcing()){
                int winningNode = longSendPool.GetCurrentMasterNode(1);
                if(winningNode >= 0){
                    pblock->payee.SetDestination(longSendMasterNodes[winningNode].pubkey.GetID());
                    
                    payments++;
                    txNew.vout.resize(payments);

                    //txNew.vout[0].scriptPubKey = scriptPubKeyIn;
                    txNew.vout[payments-1].scriptPubKey.SetDestination(longSendMasterNodes[winningNode].pubkey.GetID());
                ...
                if(blockLast.ReadFromDisk(pindexPrev)){
                    BOOST_FOREACH(CMasterNodeVote& mv1, blockLast.vmn){
                        ...
                        if(mv1.GetVotes() >= MASTERNODE_PAYMENTS_MIN_VOTES-1){
                            ...
                            BOOST_FOREACH(CMasterNodeVote& mv2, longSendMasterNodeVotes) {
                                if((mv1.blockHeight == mv2.blockHeight && mv1.GetPubKey() == mv2.GetPubKey())) {
                                    mv1.Vote();
                                    break;
                                }
                ...
                if(winningNode >= 0){
                    CMasterNodeVote mv;
                    mv.Set(longSendMasterNodes[winningNode].pubkey, pindexPrev->nHeight + 1);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("LongCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static LongCoinMiner(CWallet *pwallet)
{
    printf("LongCoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running LongCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("LongCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LongCoinMiner, pwallet));
}
...
/* *** BEGIN LONGSEND MAGIC - LONGCOIN **********
    Copyright 2014, Longcoin Developers 
        longcoindev - longcoindev@gmail.com
*/


bool CLongSendPool::SetCollateralAddress(std::string strAddress){
    CBitcoinAddress address;
    if (!address.SetString(strAddress))
    {
        printf("CLongSendPool::SetCollateralAddress - Invalid LongSend collateral address\n");
        return false;
    }
...
//Get last block hash
bool CLongSendPool::GetLastValidBlockHash(uint256& hash, int mod)
{
...
void CLongSendPool::NewBlock()
{
    if(fDebug) printf("CLongSendPool::NewBlock \n");
    ...
    {    
        ...
        if(pindexBest != NULL) {
            ...
            if(winningNode >= 0){
                CMasterNodeVote mv;
                mv.Set(longSendMasterNodes[winningNode].pubkey, pindexBest->nHeight + 1);
                longSendMasterNodeVotes.push_back(mv);

                if(longSendMasterNodeVotes.size() > MASTERNODE_PAYMENTS_EXPIRATION){
                    longSendMasterNodeVotes.erase(longSendMasterNodeVotes.begin(), longSendMasterNodeVotes.end()-MASTERNODE_PAYMENTS_EXPIRATION);
                }
            }
        }
    }
...
uint256 CMasterNode::CalculateScore(int mod)
{
    ...
    uint256 n1 = 0;
    if(!longSendPool.GetLastValidBlockHash(n1, mod)) return 0;
...
int CLongSendPool::GetCurrentMasterNode(int mod)
{
    int i = 0;
...
bool CLongSendSigner::SetKey(std::string strSecret, std::string& errorMessage, CKey& key, CPubKey& pubkey){
...
bool CLongSendSigner::SignMessage(std::string strMessage, std::string& errorMessage, vector<unsigned char>& vchSig, CKey key)
{
...
bool CLongSendSigner::VerifyMessage(CPubKey pubkey, vector<unsigned char>& vchSig, std::string strMessage, std::string& errorMessage)
{
    CHashWriter ss(SER_GETHASH, 0);
    ss << strMessageMagic;
    ss << strMessage;
